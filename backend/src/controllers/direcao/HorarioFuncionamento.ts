...
class HorarioFuncionamentoController {
    ...
    public criarHorarioFuncionamento = async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            if (!Array.isArray(diasComerciais) || !diasComerciais.every((dia: string) => /^\d{4}-\d{2}-\d{2}$/.test(dia))) {
                res.status(400).json({ erros: ["diasComerciais deve ser um array de datas no formato YYYY-MM-DD!"] });
                return;
            }

            // Converta os valores para o tipo Date
            const diasComerciaisConvertidos = diasComerciais.map((dia: string) => new Date(dia));
    ...
    public listarHorarioFuncionamento = async (req: Request, res: Response): Promise<void> => {
        try {
            const resultado = await this.conexao?.query(
    ...
    public editarHorarioFuncionamento = async (req: Request, res: Response): Promise<void> => {
        const { id } = req.params;
        const { horaInicio, horaFim, diasComerciais } = req.body;
    ...
    public excluirHorarioFuncionamento = async (req: Request, res: Response): Promise<void> => {
        const { id } = req.params;
